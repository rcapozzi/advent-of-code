...
class Grid
	...
	def crosses
		ary = []
		@grid.each do |k,node|
			ary << node if node.touch_count > 1 and node.x != 0 and node.y != 0
		end
		ary
	end
...
class Path
	...
	def distance
		net_distance = 0
		@steps.each do |direction, distance|
			offset = direction =~ /[LD]/ ? -1 : 1
